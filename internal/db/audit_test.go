...
func TestLogAction(t *testing.T) {
	...
	LogAction(nil, "test", "machine", "id1", "name1", "details", true)

	// Should insert a row
	LogAction(database, "create", "machine", "m-123", "myvm", "image=debian vcpus=2", true)
	LogAction(database, "delete", "machine", "m-123", "myvm", "", true)
	LogAction(database, "create", "machine", "m-456", "failed", "some error", false)
...
func TestAddColumnIfMissing(t *testing.T) {
	...
	var hasCol bool
	rows, _ := database.Query("PRAGMA table_info(machine)")
	for rows.Next() {
		var cid int
		var name, typ string
		var notNull int
	...
	rows.Close()
	if !hasCol {
		t.Fatal("expected network_id column in machine table")
	}

	// Calling again should not error
	addColumnIfMissing(database, "machine", "network_id", "TEXT")
}
